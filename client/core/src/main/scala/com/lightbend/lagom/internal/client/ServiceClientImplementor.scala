...
import com.lightbend.lagom.javadsl.api.{ ServiceCall, Descriptor, ServiceLocator }
...
private class ClientServiceCallInvoker[Id, Request, Response](
    ...
    circuitBreaker.withCircuitBreaker(breakerId) {
      serviceLocator.doWithService(descriptor.name, new java.util.function.Function[URI, CompletionStage[(ResponseHeader, Response)]] {
        override def apply(uri: URI) = {
          ...
          val requestHeader = requestHeaderHandler(new InternalRequestHeader(method, URI.create(url), endpoint.requestSerializer.serializerForRequest.protocol,
  ...
  private def makeStrictCall(requestHeader: RequestHeader, requestSerializer: StrictMessageSerializer[Request], responseSerializer: StrictMessageSerializer[Response],
    ...
    requestWithBody.withHeaders(acceptHeader: _*).execute().map { response =>
      ...
      val transportResponseHeader = new InternalResponseHeader(response.status, protocol, headers)
      val responseHeader = transformResponseHeader(transportResponseHeader, requestHeader)
