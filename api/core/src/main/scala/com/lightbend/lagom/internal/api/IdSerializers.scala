...
import com.lightbend.lagom.internal.api.deser.CoreIdSerializer
import com.lightbend.lagom.internal.api.deser.CoreRawId

import net.jodah.typetools.TypeResolver

trait UnresolvedIdSerializer[Id] extends CoreIdSerializer[Id] {
  def serialize(id: Id) = throw new NotImplementedError("Cannot use unresolved id serializer to serialize ids")

  def deserialize(rawId: CoreRawId) = throw new NotImplementedError("Cannot use unresolved id serializer to deserialize ids")

  def resolve(resolver: ServiceCallResolver, typeInfo: Option[Type]): CoreIdSerializer[Id]
}

class UnresolvedTypeIdSerializer[Id](val idType: Type) extends UnresolvedIdSerializer[Id] {
  override def resolve(resolver: ServiceCallResolver, typeInfo: Option[Type]): CoreIdSerializer[Id] = {
...
class CompositeFromFunctionIdSerializer[Id](name: String, paramSerializers: Seq[CoreIdSerializer[_]], deserializeF: Seq[Any] => Id,
                                            serializeF: Id => Seq[Any]) extends CoreIdSerializer[Id] {
  ...
  private val totalParams = paramSerializers.foldLeft(0) { (total, serializer) =>
    if (!serializer.numPathParamsHint.isDefined) {
  ...
  override def numPathParamsHint: Option[Int] = Option(totalParams)

  override def serialize(id: Id) = {
    paramSerializers.zip(serializeF(id)).foldLeft(CoreRawId.Empty) {
      case (rawId, (serializer, value)) =>
        val rawIdToAdd = serializer.asInstanceOf[CoreIdSerializer[Any]].serialize(value)
        CoreRawId(rawId.pathParams ++ rawIdToAdd.pathParams, rawId.queryParams ++ rawIdToAdd.queryParams)
    }
  }

  override def deserialize(rawId: CoreRawId) = {
    val (deserializedParams, _) = paramSerializers.foldLeft((Vector.empty[Any], rawId)) {
      case ((deserialized, remainingId), serializer) =>
        ...
        val newRemaingId = CoreRawId(rawId.pathParams.slice(toRemove, rawId.pathParams.size), rawId.queryParams)
...
object InternalIdSerializers {
  ...
  private def resolveFunctionArguments[F: ClassTag](f: F): Seq[Type] = {
    TypeResolver.resolveRawArguments(implicitly[ClassTag[F]].runtimeClass.asInstanceOf[Class[F]], f.getClass).dropRight(1).toList
  }

  def fromFunction[Id, Arg1](name: String, deserialize: java.util.function.Function[Arg1, Id],
                             serialize: java.util.function.Function[Id, Arg1]): CoreIdSerializer[Id] = {
  ...
  def fromBiFunction[Id, Arg1, Arg2](name: String, deserialize: java.util.function.BiFunction[Arg1, Arg2, Id],
                                     serialize: java.util.function.Function[Id, Seq[Any]]): CoreIdSerializer[Id] = {
    ...
    (3 to 22).foreach { count =>
      ...
      println(
        s"""|  def fromFunction$count[Id, $argTypeList](name: String,
            ...
            |    serialize: java.util.function.Function[Id, Seq[Any]]): IdSerializer[Id] = {
            |
  ...
  def fromFunction3[Id, Arg1, Arg2, Arg3](
    ...
    serialize:   java.util.function.Function[Id, Seq[Any]]
  ): CoreIdSerializer[Id] = {
