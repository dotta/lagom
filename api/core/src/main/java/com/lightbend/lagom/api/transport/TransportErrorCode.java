 ...
 *
 * This attempts to match up corresponding HTTP error codes with WebSocket close
 * codes, so that user code can generically select a code without worrying about
 * the underlying transport.
 *
 * While most WebSocket close codes that we typically use do have a
 * corresponding HTTP error code, there are many HTTP error codes that don't
 * have a corresponding WebSocket close code. In these cases, we use the private
 * WebSocket close code range (4xxx), with the HTTP error code as the last three
 * digits. Such WebSocket close codes will be in the range 4400 to 4599.
 *
 * This class should only be used to represent error codes, status codes like
 * HTTP 200 should not be represented using this class. This is enforced for
 * HTTP codes, since they have a well defined categorisation, codes between 400
 * and 599 are considered errors. It is however not enforced for WebSockets,
 * since the WebSocket protocol defines no such categorisation of codes, it
 * specifies a number of well known codes from 1000 to 1015, with no particular
 * pattern to their meaning, and the remaining codes are only categorised by
 * whether they are private, reserved for the WebSocket spec, or reserved for
 * applications to specify.
 *
 * For WebSocket close codes that are not known, or are not in the private range
 * of 4400 to 4599 defined by us, this use class uses the generic HTTP 404 error
 * code.
 */
public final class TransportErrorCode implements Serializable {
   ...
   */
  public static final TransportErrorCode ProtocolError = new TransportErrorCode(400, 1002,
      "Protocol Error/Bad Request");

  /**
   * An application level protocol error, such as when a client or server sent
   * data that can't be deserialized.
   */
  public static final TransportErrorCode UnsupportedData = new TransportErrorCode(400, 1003,
      "Unsupported Data/Bad Request");
  ...
  /**
   * A generic error to used to indicate that the end receiving the error
   * message violated the remote ends policy.
   */
  ...
  /**
   * The server can't generate a response that meets the clients accepted
   * response types.
   */
  ...
  /**
   * The client or server doesn't know how to deserialize the request or
   * response.
   */
  public static final TransportErrorCode UnsupportedMediaType = new TransportErrorCode(415, 4415,
      "Unsupported Media Type");

  /**
   * A generic error used to indicate that the end sending the error message
   * because it encountered an unexpected condition.
   */
  public static final TransportErrorCode UnexpectedCondition = new TransportErrorCode(500, 1011,
   ...
   */
  public static final TransportErrorCode ServiceUnavailable = new TransportErrorCode(503, 1001,
      "Going Away/Service Unavailable");
  ...
  static {
    List<TransportErrorCode> allErrorCodes = Arrays.asList(ProtocolError, UnsupportedData, Forbidden, PolicyViolation,
        MethodNotAllowed, NotAcceptable, PayloadTooLarge, UnsupportedMediaType, UnexpectedCondition,
        ServiceUnavailable);
   ...
   *
   * This description will be meaningful for known built in close codes, but for
   * other codes, it will be {@code "Unknown error code"}.
   *
  ...
  public String toString() {
    return "TransportErrorCode{" + "http=" + http + ", webSocket=" + webSocket + ", description='" + description + '\''
        + '}';
  }
}
