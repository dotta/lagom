...
object ServiceReader {
  ...
  def resolveServiceDescriptor(descriptor: Descriptor, classLoader: ClassLoader,
    ...
    // FIXME: Lots of duplication with `endpoints` impl to streamline.

    val topics = descriptor.topicCalls().asScala.map { tc =>
      val topicCall = tc.asInstanceOf[TopicCall[Any]]
      val methodRefTopicSourceHolder = topicCall.topicCallHolder() match {
        case methodRef: MethodRefTopicHolder => methodRef
        case other                           => throw new IllegalArgumentException("Unknown TopicCallHolder type: " + other)
      }
      ...
      topicCall
        ...
        // FIXME: Shall we create and use a different resolver?
        .withMessageSerializer(serviceResolver.resolveMessageSerializer(topicCall.messageSerializer(), topicMessageType))
        .withMessagePayloadType(topicMessageType) // FIXME: withMessagePayloadType should not be needed...
    }
  ...
  private def constructTopicHolder(serviceCallResolver: ServiceCallResolver, _method: Method, topicId: TopicId): TopicHolder = {
    new MethodTopicHolder {
      ...
      override def create(service: Any): Topic[_] = {
        method.invoke(service).asInstanceOf[TopicSource[_]].withTopicId(topicId)
      }
    }
  }
