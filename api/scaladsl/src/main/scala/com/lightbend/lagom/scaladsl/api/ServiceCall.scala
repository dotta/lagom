 ...
 * using json.
 */
trait ServiceCall[Id, Request, Response] { self =>
   ...
   */
  def handleRequestHeader(handler: RequestHeader => RequestHeader): ServiceCall[Id, Request, Response] = {
   ...
   */
  def handleResponseHeader[T](handler: (ResponseHeader, Response) => T)(implicit ec: ExecutionContext): ServiceCall[Id, Request, T] = new ServiceCall[Id, Request, T] {
    // Default implementation. For client service calls, this is overridden by the implementation to do something
    // with the handler.
    def apply(id: Id, request: Request) =
      self(id, request).map(response => handler(ResponseHeader.Ok, response))
  }
   ...
   */
  def withResponseHeader(implicit ec: ExecutionContext): ServiceCall[Id, Request, (ResponseHeader, Response)] = {
